...
function LocationRows({
  ...
  return <table className="min-w-full divide-y divide-slate-200"><thead className="bg-slate-50"><tr className="text-left text-xs font-semibold uppercase tracking-[0.16em] text-slate-500"><th className="px-4 py-3">Location</th><th className="px-4 py-3">Warehouse</th><th className="px-4 py-3">Coordinates</th><th className="px-4 py-3">Load</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-200 bg-white">{items.map((item) => <tr key={item.id} className="align-top"><td className="px-4 py-4"><div className="font-semibold text-slate-900">{item.code}</div><div className="mt-1 text-xs text-slate-400">{item.productCount} products</div></td><td className="px-4 py-4 text-sm text-slate-600">{item.warehouseName}</td><td className="px-4 py-4 text-sm text-slate-600">Zone {item.zone} · Aisle {item.aisle} · Rack {item.rack} · Level {item.level} · Bin {item.bin}</td><td className="px-4 py-4 text-sm text-slate-600">{item.currentLoad} / {item.capacity}</td><td className="px-4 py-4"><StatusBadge status={item.status} /></td><td className="px-4 py-4"><div className="flex justify-end gap-2"><ActionButton icon="visibility" label="View" onClick={() => onView(item)} />{canManage ? <ActionButton icon="edit" label="Edit" onClick={() => onEdit(item)} /> : null}{canManage ? <ActionButton icon="delete" label="Delete" danger onClick={() => onDelete(item)} /> : null}</div></td></tr>)}</tbody></table>;
}
