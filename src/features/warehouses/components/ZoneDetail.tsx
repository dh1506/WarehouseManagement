...
import { useUpdateZoneBinCapacity, useWarehouseHubs, useZoneBins } from '../hooks/useWarehouses';
...
function getLegendLabel(level: BinOccupancyLevel) {
  ...
  if (level === 'partial') return 'Partial (1-60%)';
...
export function ZoneDetail() {
  ...
  const levelList = useMemo(
    () => Array.from(new Set(bins.map((bin) => bin.level))).sort((a, b) => b - a),
    [bins],
  );
  const [level, setLevel] = useState<number>(levelList[0] ?? 1);
  ...
  const binsOfLevel = useMemo(() => bins.filter((bin) => bin.level === level), [bins, level]);
  const rows = useMemo(() => Array.from(new Set(binsOfLevel.map((bin) => bin.row))).sort((a, b) => a - b), [binsOfLevel]);

  const selectedBin = useMemo(() => {
    if (!selectedBinId) return binsOfLevel[0] ?? null;
    return bins.find((bin) => bin.id === selectedBinId) ?? binsOfLevel[0] ?? null;
  }, [bins, binsOfLevel, selectedBinId]);
  ...
  const { register, reset, handleSubmit, setValue, watch, formState: { errors } } = form;
      ...
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-2 shadow-sm">
            ...
            <span className="text-xl font-bold text-blue-700">{zone.occupancy}%</span>
            <div className="h-2 w-16 overflow-hidden rounded-full bg-slate-200">
              <div className="h-full bg-blue-700" style={{ width: `${zone.occupancy}%` }} />
            </div>
          </div>
        ...
        <div className="space-y-6 xl:col-span-8">
          <div className="rounded-[2rem] bg-white p-6 shadow-sm sm:p-8">
            <div className="mb-8 flex flex-wrap gap-6 border-b border-slate-200 pb-6">
              {(['empty', 'partial', 'full', 'overloaded'] as BinOccupancyLevel[]).map((levelValue) => (
            ...
            <div className="mb-6 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
              <div>
                <p className="mb-2 text-sm font-semibold text-slate-600">Storage Level</p>
                <div className="flex flex-wrap gap-2">
                  {levelList.map((item) => (
                    <button
                      ...
                      className={`rounded-lg px-4 py-2 text-sm font-bold transition ${level === item ? 'bg-blue-700 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                        }`}
                    >
                      Level {item}
                    </button>
            ...
            {viewMode === 'grid' ? (
              <div className="overflow-x-auto pb-3">
                <div className="min-w-170">
                  <div className="grid gap-8" style={{ gridTemplateColumns: `repeat(${rows.length}, minmax(200px, 1fr))` }}>
                    {rows.map((row) => (
                      <div key={row} className="space-y-4">
                        <p className="text-center text-lg font-bold text-slate-900">ROW {String.fromCharCode(64 + row)}</p>
                        <div className="space-y-2">
                          {chunkArray(
                            binsOfLevel
                              .filter((bin) => bin.row === row)
                              .sort((a, b) => a.shelf - b.shelf),
                            4,
                          ).map((shelfLine, shelfLineIndex) => (
                            <div key={`shelf-${row}-${shelfLineIndex}`} className="grid grid-cols-4 gap-2">
                              {shelfLine.map((bin) => (
                                <button
                                  key={bin.id}
                                  type="button"
                                  onClick={() => setSelectedBinId(bin.id)}
                                  className={`flex h-12 w-full items-center justify-center rounded-lg text-[11px] font-bold leading-tight ring-offset-2 transition hover:ring-2 ${getOccupancyColor(bin.occupancyLevel)} ${selectedBin?.id === bin.id ? 'ring-2 ring-blue-700' : ''}`}
                                  title={`Bin ${bin.code}`}
                                >
                                  {bin.code}
                                </button>
