...
export const warehouseLocationFormSchema = z
  .object({
    ...
    zoneCode: z.string().trim().min(1, 'Vui lòng nhập zone').max(20, 'Zone tối đa 20 ký tự'),
    aisleCode: z.string().trim().min(1, 'Vui lòng nhập aisle').max(20, 'Aisle tối đa 20 ký tự'),
    rackCode: z.string().trim().min(1, 'Vui lòng nhập rack').max(20, 'Rack tối đa 20 ký tự'),
    levelCode: z.string().trim().min(1, 'Vui lòng nhập level').max(20, 'Level tối đa 20 ký tự'),
    binCode: z.string().trim().min(1, 'Vui lòng nhập bin').max(20, 'Bin tối đa 20 ký tự'),
    status: z.enum(['available', 'partial', 'full', 'maintenance']),
    isActive: z.boolean(),
    maxWeight: z.number().nullable(),
    maxVolume: z.number().nullable(),
    currentWeight: z.number().nullable(),
    currentVolume: z.number().nullable(),
    storageCondition: z.enum(['ambient', 'chilled', 'frozen', 'dry']),
  })
  .refine((data) => data.maxWeight === null || data.maxWeight >= 0, {
    path: ['maxWeight'],
    message: 'Khối lượng tối đa không hợp lệ',
  })
  .refine((data) => data.maxVolume === null || data.maxVolume >= 0, {
    path: ['maxVolume'],
    message: 'Thể tích tối đa không hợp lệ',
  })
  .refine((data) => data.currentWeight === null || data.currentWeight >= 0, {
    path: ['currentWeight'],
    message: 'Khối lượng hiện tại không hợp lệ',
  })
  .refine((data) => data.currentVolume === null || data.currentVolume >= 0, {
    path: ['currentVolume'],
    message: 'Thể tích hiện tại không hợp lệ',
  })
  .refine((data) => data.maxWeight === null || data.currentWeight === null || data.currentWeight <= data.maxWeight, {
    path: ['currentWeight'],
    message: 'Khối lượng hiện tại không được vượt quá khối lượng tối đa',
  })
  .refine((data) => data.maxVolume === null || data.currentVolume === null || data.currentVolume <= data.maxVolume, {
    path: ['currentVolume'],
    message: 'Thể tích hiện tại không được vượt quá thể tích tối đa',
  });
