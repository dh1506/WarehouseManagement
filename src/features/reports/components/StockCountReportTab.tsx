...
import type { StockCountReportParams } from '../types/reportType';
...
function DiscrepancyBadge({ details }: { details: { variance_quantity: string | null }[] }) {
  const count = details.filter(
    (d) => d.variance_quantity !== null && Number(d.variance_quantity) !== 0,
  ).length;
  if (count === 0) {
    return <span className="text-xs text-emerald-600 font-medium">Không có sai lệch</span>;
  }
  return (
...
export function StockCountReportTab() {
  const navigate = useNavigate();
  const [page, setPage] = useState(1);
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');

  const params: StockCountReportParams = {
    page,
    limit: PAGE_SIZE,
  ...
  const { data, isLoading, isError } = useStockCountReport(params);
          ...
          <table className="w-full text-sm min-w-[750px]">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/70 sticky top-0 z-10">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Loại kiểm kê</th>
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">Tổng dòng</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Sai lệch</th>
            ...
            <tbody>
              ...
              {!isLoading && !isError && data?.data.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <span className="material-symbols-outlined text-[28px] text-slate-300">fact_check</span>
                      <p className="text-sm text-slate-400">Không có dữ liệu kiểm kê trong kỳ này</p>
                    </div>
                  </td>
                </tr>
              )}
