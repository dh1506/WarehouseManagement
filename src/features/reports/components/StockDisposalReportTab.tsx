...
function totalQty(details: { quantity: string }[]): string {
  const sum = details.reduce((acc, d) => acc + Number(d.quantity), 0);
...
export function StockDisposalReportTab() {
  const navigate = useNavigate();
  const [page, setPage] = useState(1);
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');

  const params: StockDisposalReportParams = {
    page,
    limit: PAGE_SIZE,
  ...
  const { data, isLoading, isError } = useStockDisposalReport(params);
          ...
          <table className="w-full text-sm min-w-[700px]">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/70 sticky top-0 z-10">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Lý do thanh lý</th>
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">Tổng SL thanh lý</th>
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">Số dòng</th>
            ...
            <tbody>
              ...
              {!isLoading && !isError && data?.data.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <span className="material-symbols-outlined text-[28px] text-slate-300">delete_sweep</span>
                      <p className="text-sm text-slate-400">Không có dữ liệu thanh lý trong kỳ này</p>
                    </div>
                  </td>
                </tr>
              )}
              {!isLoading && !isError && data?.data.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-slate-600 text-xs max-w-[200px] truncate">{reasons || '—'}</td>
                    ...
                    <td className="px-4 py-3">
                      <button
                        type="button"
                        onClick={() => navigate(`/stock-disposal/${item.id}`)}
