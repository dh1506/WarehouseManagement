...
import { useStockOutReport } from '../hooks/useReports';
import type { StockOutReportParams } from '../types/reportType';
...
function TypeBadge({ type }: { type: string }) {
  const isReturn = type === 'RETURN_TO_SUPPLIER';
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${
        isReturn ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'
      }`}
    >
      {isReturn ? 'Hoàn trả' : 'Bán hàng'}
    </span>
  );
}
...
export function StockOutReportTab() {
  const navigate = useNavigate();
  const [page, setPage] = useState(1);
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');

  const params: StockOutReportParams = {
    page,
    limit: PAGE_SIZE,
  ...
  const { data, isLoading, isError } = useStockOutReport(params);
          ...
          <table className="w-full text-sm min-w-[700px]">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/70 sticky top-0 z-10">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Vị trí xuất</th>
            ...
            <tbody>
              ...
              {!isLoading && !isError && data?.data.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <span className="material-symbols-outlined text-[28px] text-slate-300">local_shipping</span>
              ...
              {!isLoading && !isError && data?.data.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-slate-600 text-xs">
                    {item.location?.full_path ?? item.location?.location_code ?? '—'}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <button
                      type="button"
                      onClick={() => navigate(`/outbound/${item.id}`)}
