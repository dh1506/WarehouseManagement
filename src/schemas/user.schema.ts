...
export const createUserSchema = z.object({
  body: z.object({
    ...
    full_name: z.string().min(1, 'Họ tên không được để trống').max(100, 'Họ tên tối đa 100 ký tự'),
    email: z.string().email('Email không hợp lệ').trim().toLowerCase().optional(),
    phone: z
      .string()
...
export const updateUserSchema = z.object({
  ...
  body: z.object({
    full_name: z.string().min(1, 'Họ tên không được để trống').max(100, 'Họ tên tối đa 100 ký tự').optional(),
    email: z.string().email('Email không hợp lệ').trim().toLowerCase().optional().nullable(),
    phone: z
      .string()
      .regex(/^[0-9+]+$/, 'Số điện thoại không hợp lệ')
      .optional()
    ...
    role_id: z.number().int().positive('ID vai trò phải là số nguyên dương').optional(),
    user_status: z.enum(['ACTIVE', 'INACTIVE', 'SUSPENDED']).optional(),
    password: z
      .string()
      .min(6, 'Mật khẩu phải có ít nhất 6 ký tự')
